    ...
    if rd < 0.3: 
        ...
        while True:              
            ...
            if f_w < w:
                ...
                size1, size2 = crop_img.size
                if size1 < 5 or size2 < 5:
                    print("------h------------> {} --- > {}". format(size1, size2))
                    continue
...
if __name__ == '__main__':
    ...
    parser.add_argument('--num_img', type=int, default=30000, help="Number of images to generate")
    
    parser.add_argument('--font_min_size', type=int, default=20)      #12
    ...
    parser.add_argument('--bg_path', type=str, default='./background/cimc',
                        help='The generated text pictures will use the pictures of this folder as the background')
                        
    parser.add_argument('--fonts_path',type=str, default='./fonts/cimc',     #chinse_jian
    ...
    parser.add_argument('--blur', action='store_true', default= False,         #False,
                        help="Apply gauss blur to the generated image")    
    
    parser.add_argument('--prydown', action='store_true', default=True,       #False
                    help="Blurred image, simulating the effect of enlargement of small pictures")

    parser.add_argument('--lr_motion', action='store_true', default= True,       #False,
                    help="Apply left and right motion blur")
                    
    parser.add_argument('--ud_motion', action='store_true', default=False,  #False
    ...
    parser.add_argument('--output_dir', type=str, default='./output/synth_word1131/', help='Images save dir')
    ...
    # import matplotlib.pyplot as plt
    labels_path = 'labels_cimc1131.txt'
    gs = 0
    ...
    for i in range(gs+1,cf.num_img):
        ...
        if cf.prydown:
            ...
            gen_img = Image.fromarray(np.uint8(gen_img))
        #左右运动模糊
        lr_rnd = random.random()
        if cf.lr_motion and lr_rnd < 0.3:
            image_arr = np.array(gen_img)
